...
import { addOrderAddress, addPayment, getAlladdress } from "../../Services/buyer";
...
export default function BuyerShippingandPayment() {
  ...
  const onConfirmPayment = () => {
    if (!payment) {
      setIsRed(true);
      toast.error('Payment proof not uploaded');
      return;
    }
    const paymentObj ={ orderId:orderSummary.id, paymentDetails:{paymentMethod:'UPI', amount:orderSummary.totalAmount, paymentScreenshot:payment}}

    // console.log('final maaal', paymentObj);
    setIsloading(true)
  ...
  <div className="flex flex-1 p-6 gap-4">
      ...
      <div>
        {orderSummary ? (
          <div className="space-y-4">
            ...
            <p>Order Date: {new Date(orderSummary.orderDate).toLocaleString()}</p>
            <p>Total Amount: ₹{orderSummary.totalAmount}</p>
            {/* <p>Payment Status: {orderSummary.paymentStatus === 1 ? "Completed" : "Pending"}</p> */}

            <h3 className="text-lg font-semibold">Shipping Address</h3>
            <p>{orderSummary.orderAddress.addressLine1}, {orderSummary.orderAddress.addressLine2}</p>
            <p>{orderSummary.orderAddress.city}, {orderSummary.orderAddress.state}, {orderSummary.orderAddress.pinCode}</p>
            ...
            {orderSummary.appliedOffer && (
              <div>
                ...
                <p>Description: {orderSummary.appliedOffer.offerDescription}</p>
              </div>
            )}
          </div>
        ) : (
    ...
    <div className="w-96 bg-white shadow-md p-4 flex flex-col justify-between">
      <div>
        ...
        <p className=" mb-2">Total payable amount {orderSummary && <span className=" font-bold">₹ {orderSummary.totalAmount.toFixed(2)}/-</span>} </p>
        <input
          type="file"
          accept="image/*"
